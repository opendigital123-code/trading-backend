...
// ============================================================
// CORS — permissif par defaut, restreindre via env CORS_ORIGIN
// ============================================================
...
// ============================================================
// TIMEOUTS — augmentes pour Render cold starts
// ============================================================
...
// HTTP CLIENT
// ============================================================
// Plusieurs User-Agents rotatifs pour contourner les blocages
const USER_AGENTS = [
...
// Injecte un User-Agent rotatif sur chaque requete
...
const LOCKS_DUMP_FILE      = process.env.LOCKS_DUMP_FILE || path.join('/tmp', '.signal_locks.json');
...
function resolveSignal(key, newAnalysis) {
  ...
  if (
    ...
    const lock = {
      ...
      expiresAt:   now + SIGNAL_LOCK_MS,
      rawAnalysis: newAnalysis,
      overrideReason: `Override: confiance ${newAnalysis.confidence}% > seuil ${SIGNAL_OVERRIDE_CONF}%`
    };
    SIGNAL_LOCKS.set(key, lock);
    return buildLockedResponse(lock, newAnalysis, now);
  }
...
function buildLockedResponse(lock, freshAnalysis, now) {
  ...
  return {
    ...
    reasons:    isLocked
      ? [...lock.rawAnalysis.reasons, `Signal verrouille — ${minutesRemaining}m${secsDisplay}s restantes`]
...
function calculateMACD(prices, fast = 12, slow = 26, signal = 9) {
  ...
  if (validMacd.length < signal) {
    return { macd: 0, signal: 0, histogram: 0, prevHistogram: 0, bullCross: false, bearCross: false, histogramSeries: macdLine.map(() => null) };
  }
  ...
  const macdOffset   = macdLine.length - validMacd.length;
  ...
  return { macd: lastMacd, signal: lastSig, histogram: lastMacd - lastSig, prevHistogram: prevMacd - prevSigVal, bullCross, bearCross, histogramSeries };
}
...
function detectCandlePatterns(candles) {
  ...
  const bullish = [], bearish = [], confirmationDetails = [];
  ...
  if (lowerWick1 >= body1 * 2 && upperWick1 <= body1 * 0.5 && range1 > 0 && trendBearish) {
    bullish.push('Hammer/Pin Bar'); bullScore += 7;
    if (isBull0 && c0.close > (c1.high + c1.low) / 2) { bullScore += 5; confirmed = true; confirmationDetails.push('Hammer confirme par bougie haussiere suivante'); }
  }
  if (upperWick1 >= body1 * 2 && lowerWick1 <= body1 * 0.5 && range1 > 0 && trendBullish) {
    bearish.push('Shooting Star'); bearScore += 7;
    if (isBear0 && c0.close < (c1.high + c1.low) / 2) { bearScore += 5; confirmed = true; confirmationDetails.push('Shooting Star confirme'); }
  }
  if (upperWick1 >= body1 * 2 && lowerWick1 <= body1 * 0.5 && range1 > 0 && trendBearish) {
    bullish.push('Inverted Hammer'); bullScore += 5;
    if (isBull0 && c0.close > c1.high) { bullScore += 4; confirmed = true; confirmationDetails.push('Inverted Hammer confirme — cloture au-dessus du high'); }
  }
  if (lowerWick1 >= body1 * 2 && upperWick1 <= body1 * 0.5 && range1 > 0 && trendBullish) {
    bearish.push('Hanging Man'); bearScore += 5;
    if (isBear0 && c0.close < c1.low) { bearScore += 4; confirmed = true; confirmationDetails.push('Hanging Man confirme — cloture sous le low'); }
  }
  if (isBull1 && upperWick1 < body1 * 0.05 && lowerWick1 < body1 * 0.05 && body1 > avgBody * 1.2) {
    bullish.push('Marubozu Haussier'); bullScore += 6;
    if (isBull0) { bullScore += 3; confirmationDetails.push("Marubozu haussier suivi d'une cloture positive"); }
  }
  if (isBear1 && upperWick1 < body1 * 0.05 && lowerWick1 < body1 * 0.05 && body1 > avgBody * 1.2) {
    bearish.push('Marubozu Baissier'); bearScore += 6;
    if (isBear0) { bearScore += 3; confirmationDetails.push("Marubozu baissier suivi d'une cloture negative"); }
  }
  if (body1 < range1 * 0.10) { bullish.push('Doji'); bearish.push('Doji'); }
  if (isBear2 && isBull1 && c1.open <= c2.close && c1.close >= c2.open && body1 > body2 * 0.9) {
    bullish.push('Engulfing Haussier'); bullScore += 9;
    if (isBull0 && c0.close > c1.close) { bullScore += 5; confirmed = true; confirmationDetails.push('Engulfing haussier confirme'); }
  }
  if (isBull2 && isBear1 && c1.open >= c2.close && c1.close <= c2.open && body1 > body2 * 0.9) {
    bearish.push('Engulfing Baissier'); bearScore += 9;
    if (isBear0 && c0.close < c1.close) { bearScore += 5; confirmed = true; confirmationDetails.push('Engulfing baissier confirme'); }
  }
  if (isBear2 && isBull1 && Math.abs(c1.low - c2.low) / (range1 || 0.0001) < 0.08) {
    bullish.push('Tweezer Bottom'); bullScore += 6;
    if (isBull0) { bullScore += 3; confirmationDetails.push('Tweezer Bottom confirme'); }
  }
  if (isBull2 && isBear1 && Math.abs(c1.high - c2.high) / (range1 || 0.0001) < 0.08) {
    bearish.push('Tweezer Top'); bearScore += 6;
    if (isBear0) { bearScore += 3; confirmationDetails.push('Tweezer Top confirme'); }
  }
  if (isBear2 && isBull1 && c1.open > c2.close && c1.close < c2.open && body1 < body2 * 0.5) {
    bullish.push('Harami Haussier'); bullScore += 5;
    if (isBull0 && c0.close > c1.close) { bullScore += 4; confirmed = true; confirmationDetails.push('Harami haussier confirme'); }
  }
  if (isBull2 && isBear1 && c1.open < c2.close && c1.close > c2.open && body1 < body2 * 0.5) {
    bearish.push('Harami Baissier'); bearScore += 5;
    if (isBear0 && c0.close < c1.close) { bearScore += 4; confirmed = true; confirmationDetails.push('Harami baissier confirme'); }
  }
  if (isBear2 && isBull1 && c1.open < c2.low && c1.close > (c2.open + c2.close) / 2 && c1.close < c2.open) {
    bullish.push('Piercing Line'); bullScore += 7;
    if (isBull0) { bullScore += 3; confirmed = true; confirmationDetails.push('Piercing Line confirme'); }
  }
  if (isBull2 && isBear1 && c1.open > c2.high && c1.close < (c2.open + c2.close) / 2 && c1.close > c2.open) {
    bearish.push('Dark Cloud Cover'); bearScore += 7;
    if (isBear0) { bearScore += 3; confirmed = true; confirmationDetails.push('Dark Cloud Cover confirme'); }
  }
  if (isBear3 && body3 > avgBody && Math.abs(c2.close - c2.open) < range1 * 0.3 && isBull1 && c1.close > (c3.open + c3.close) / 2) {
    bullish.push('Morning Star'); bullScore += 10;
    if (isBull0 && c0.close > c1.close) { bullScore += 5; confirmed = true; confirmationDetails.push('Morning Star confirme'); }
  }
  if (isBull3 && body3 > avgBody && Math.abs(c2.close - c2.open) < range1 * 0.3 && isBear1 && c1.close < (c3.open + c3.close) / 2) {
    bearish.push('Evening Star'); bearScore += 10;
    if (isBear0 && c0.close < c1.close) { bearScore += 5; confirmed = true; confirmationDetails.push('Evening Star confirme'); }
  }
  if (isBull3 && isBull2 && isBull1 && c1.close > c2.close && c2.close > c3.close && body1 > avgBody * 0.7 && body2 > avgBody * 0.7 && body3 > avgBody * 0.7 && lowerWick1 < body1 * 0.3) {
    bullish.push('Three White Soldiers'); bullScore += 11;
    if (isBull0) { bullScore += 4; confirmed = true; confirmationDetails.push('Three White Soldiers confirmes'); }
  }
  if (isBear3 && isBear2 && isBear1 && c1.close < c2.close && c2.close < c3.close && body1 > avgBody * 0.7 && body2 > avgBody * 0.7 && body3 > avgBody * 0.7 && upperWick1 < body1 * 0.3) {
    bearish.push('Three Black Crows'); bearScore += 11;
    if (isBear0) { bearScore += 4; confirmed = true; confirmationDetails.push('Three Black Crows confirmes'); }
  }

  return { bullish: [...new Set(bullish)], bearish: [...new Set(bearish)], bullScore, bearScore, confirmed, confirmationDetails };
}
...
function normalizeNewsScore(news = '') {
  const text = String(news).slice(0, 500).toLowerCase();
...
function createScalpingTradePlan({ signal, score, price, atr, support, resistance, bb }) {
  ...
  return {
    ...
    exitPlan: [
      ...
      'TP3 (3.5R): sortie totale ou trailing stop de 0.5x ATR.'
    ].join(' | ')
  };
}
...
function generateScalpingSignal(candles, options = {}) {
  ...
  if (candles.length < MIN_CANDLES) {
    return { signal: 'HOLD', strength: 'FAIBLE', confidence: 0, score: 0, confirmations: 0, reasons: ['Donnees insuffisantes pour analyse fiable'], indicators: {}, tradePlan: null };
  }
  ...
  if (Number.isFinite(ema200)) { if (price > ema200) score += 8; else score -= 8; }
  ...
  // VWAP
  if (price > vwapData.upper1)      { score += 5; reasons.push('Prix au-dessus VWAP +1s'); }
  else if (price > vwap)            { score += 7; }
  else if (price < vwapData.lower1) { score -= 5; reasons.push('Prix en-dessous VWAP -1s'); }
  else if (price < vwap)            { score -= 7; }
  ...
  else if (rsi14 > 72)                 { score -= 6; reasons.push('RSI suracheté (>72)'); }
  else if (rsi14 < 28)                 { score += 6; reasons.push('RSI survendu (<28)'); }
  ...
  else if (stochRsi.k > 80 && stochRsi.d > 80) { score -= 5; reasons.push('StochRSI suracheté'); }
  else if (stochRsi.k < 20 && stochRsi.d < 20) { score += 5; reasons.push('StochRSI survendu'); }

  // BOLLINGER BANDS
  if      (bb.percentB < 0.05)                        { score += 7; reasons.push('Prix sur bande BB inferieure'); confirmations++; }
  else if (bb.percentB > 0.95)                        { score -= 7; reasons.push('Prix sur bande BB superieure'); confirmations++; }
  else if (bb.percentB > 0.55 && price > bb.middle)  { score += 4; }
  else if (bb.percentB < 0.45 && price < bb.middle)  { score -= 4; }

  // SQUEEZE
  if (squeeze.squeezeOn) {
    if (squeeze.squeezeBullish) { score += 6; reasons.push('Squeeze BB — biais haussier'); }
    else                        { score -= 6; reasons.push('Squeeze BB — biais baissier'); }
  }
  ...
  if (candlePatterns.bullScore > 0 || candlePatterns.bearScore > 0) {
    ...
    if (bullPatternsFiltered.length) { reasons.push(`Pattern(s) haussier(s): ${bullPatternsFiltered.join(', ')}`); confirmations++; }
    if (bearPatternsFiltered.length) { reasons.push(`Pattern(s) baissier(s): ${bearPatternsFiltered.join(', ')}`); confirmations++; }
    if (candlePatterns.confirmationDetails.length) {
      for (const detail of candlePatterns.confirmationDetails) reasons.push(detail);
  ...
  else if (atrPercent < 0.05) { score *= 0.85; reasons.push('Volatilite trop basse — risque slippage'); }
  if (rsi7 > 80 && score > 0) { score *= 0.80; reasons.push('RSI7 suracheté extremement'); }
  if (rsi7 < 20 && score < 0) { score *= 0.80; reasons.push('RSI7 survendu extremement'); }
...
async function fetchYahooMarket(rawSymbol, type, timeConfig) {
  ...
  // Essayer les deux endpoints Yahoo (v8 et v7 en fallback)
  const endpoints = [
...
// ── DIAGNOSTIC ROUTE — teste chaque source séparément ──────
app.get('/diagnose', async (req, res) => {
  const { symbol = 'BTCUSDT', interval = '15m' } = req.query;
  const timeConfig = INTERVAL_MAP[interval] || INTERVAL_MAP['15m'];
  const pair       = normalizeCryptoPair(symbol);
  const results    = {};

  const sources = [
    { name: 'binance',  fn: () => fetchBinanceCrypto(pair, timeConfig) },
    { name: 'bybit',    fn: () => fetchBybitCrypto(pair, timeConfig)   },
    { name: 'coinbase', fn: () => fetchCoinbaseCrypto(pair, timeConfig) }
  ];

  await Promise.allSettled(sources.map(async ({ name, fn }) => {
    try {
      const data = await fn();
      results[name] = { ok: true, candles: data.candles.length, lastPrice: data.currentPrice, source: data.source };
    } catch (err) {
      results[name] = { ok: false, error: err.message };
    }
  }));

  res.json({ symbol, interval, results, timestamp: new Date().toISOString() });
});
...
app.get('/market', async (req, res) => {
  const timer = setTimeout(() => {
    if (!res.headersSent) {
      res.status(504).json({ error: 'Timeout — aucune source n\'a repondu dans les delais.' });
    }
  }, ROUTE_TIMEOUT_MS);
  ...
  try {
    ...
    const currentPrice = Number.isFinite(market.currentPrice) && market.currentPrice > 0
      ? market.currentPrice
...
app.listen(PORT, HOST, () => {
  console.log(`[server] Ecoute sur ${HOST}:${PORT}`);
  console.log(`[config] Signal lock: ${SIGNAL_LOCK_MS / 1000}s | Min confirmations: ${SIGNAL_MIN_CONFIRM} | Override: ${SIGNAL_OVERRIDE_CONF}%`);
  console.log(`[config] Cache LRU max ${CACHE_MAX_SIZE} | Route timeout: ${ROUTE_TIMEOUT_MS / 1000}s | Axios timeout: ${AXIOS_TIMEOUT / 1000}s`);
});
